...
import { getRouletteTableById, updateTable } from 'services/tables';
...
function EditRouletteTable() {
  ...
  useEffect(() => {
    getRouletteTableById(id).then((response) => {
      const { data } = response;
      setInitalValues({
  ...
  const submitForm = async (values, actions) => {
    const response = await updateTable('roulette', id, values.min_bet, values.max_bet);
