...
import { getCrashTable, updateCrashTable } from 'services/crash';
...
function EditCrashTable() {
  ...
  useEffect(() => {
    getCrashTable(id).then((response) => {
      const { data } = response;
      setInitialValues({
